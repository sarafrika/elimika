...
public class CurrencyController {
    ...
    @GetMapping
    @Operation(summary = "List active platform currencies (paginated)")
    public ResponseEntity<ApiResponse<PagedDTO<CurrencyDTO>>> listActiveCurrencies(Pageable pageable) {
        Page<CurrencyDTO> currencies = currencyService.getActiveCurrencies(pageable);
        PagedDTO<CurrencyDTO> payload = PagedDTO.from(
                currencies,
                ServletUriComponentsBuilder.fromCurrentRequestUri().build().toString()
        );
        return ResponseEntity.ok(ApiResponse.success(payload, "Active currencies retrieved successfully"));
    }
