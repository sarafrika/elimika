...
/**
 * Service for validating instructor document files before upload.
 * Enforces file type and size constraints for instructor credentials and certificates.
 */
@Service
@RequiredArgsConstructor
public class InstructorDocumentValidationService {
    ...
    /**
     * Validates an instructor document file.
    ...
    private void validatePdfType(MultipartFile file) {
        ...
        if (!PDF_CONTENT_TYPE.equals(contentType)) {
            throw new IllegalArgumentException("Only PDF files are allowed for instructor documents");
        }
    }
