CREATE OR REPLACE FUNCTION generate_student_full_name_from_user()
    RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER trigger_generate_student_full_name_from_user
    BEFORE INSERT OR UPDATE ON students
    FOR EACH ROW
    EXECUTE FUNCTION generate_student_full_name_from_user();

-- Update students.full_name when users table is updated
CREATE OR REPLACE FUNCTION update_student_full_name_on_user_change()
    RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_student_full_name_on_user_change
    AFTER UPDATE OF first_name, middle_name, last_name ON users
    FOR EACH ROW
EXECUTE FUNCTION update_student_full_name_on_user_change();