...
## Testing Guidelines
Place integration scenarios under `src/test/java/.../integration` and prefer Testcontainers for external dependencies. Run `./gradlew test` before any pull request and document notable new assertions, especially around Flyway migrations and critical business rules. Keep the Spring Modulith architecture tests active and ensure the project stays fully compliant with Spring Modulith role rules; reintroduce other unit tests only once they align with the refined module boundaries.

## Commit & Pull Request Guidelines
Follow Conventional Commits (`feat:`, `fix:`, `chore:`) as seen in `feat: enforce commerce paywall before class enrollment`. Write a **single-line** subject and nothing else — no body, no bullet list, and never a `Co-Authored-By` attribution trailer. Say what changed and where the scope is, in the imperative; the diff carries the detail. Keep each commit focused.

Pull requests should include a brief summary, linked issue, test evidence (`./gradlew test` output or UI screenshots), and call out configuration or secret changes so reviewers can coordinate updates.
...
- Some pushes go **straight to `main`** rather than through a branch — confirm the expectation before committing rather than assuming a feature branch.
- If a branch *is* expected and work started on the wrong one, stash the changes, create the correct branch, re-apply the stash, then commit and open the PR from that branch.
