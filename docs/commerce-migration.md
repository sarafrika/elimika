...
2) **Domain Services (flagged)**: implement internal catalogue lookup, cart lifecycle, checkout/order state machine, and payment provider abstraction behind the flag.
3) **API Layer (flagged)**: expose `/api/v1/commerce/catalogue|carts|orders` backed by internal services while preserving Medusa endpoints for fallback.
...
5) **Cutover**: toggle flag in non-prod, monitor metrics/logs, then enable in production once carts/checkout/orders pass validation. Add migration for any initial catalogue seed data if needed.
6) **Cleanup**: remove Medusa configuration/clients/DTOs and drop Medusa-specific tables/columns once stable.

## Data Model (new tables)
- `commerce_product`, `commerce_product_variant`: catalogue and pricing/inventory metadata linked to courses/classes.
